...
local function safeLoadstring(url, noCache)
    local success, result = pcall(function()
        local content = game:HttpGet(url, noCache)
        if not content or content == "" then
            warn("Empty content from: " .. url)
            return nil
        end
...
safeLoadstring('https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/alebinh60-asmobile/anhyeuvy', true)
safeLoadstring('https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/yuvic123-SKIDO-V3/fixed%20fireclick', true)

local placeId = game.PlaceId

if placeId == 17887390746 or placeId == 17887392216 or placeId == 17793786340 or placeId == 14202073004 then
    safeLoadstring('https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/alebinh60-asmobile/unnamed', true)
...
if getnamecallmethod then
    safeLoadstring('https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/Pixeluted-adoniscries/Source.lua', true)
end
...
local repo = 'https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/Kazamatcha-asmobile/'
...
local Library = safeLoadLibrary()
if not Library then
    error("Could not load Library - halting script")
end

local function safeLoadAddon(path)
    local success, result = pcall(function()
        return loadstring(game:HttpGet(repo .. path, true))()
    end)
    if not success then
        warn("Failed to load addon " .. path .. ": " .. tostring(result))
        return nil
    end
    return result
end

local ThemeManager = safeLoadAddon('addons/ThemeManager.lua')
local SaveManager = safeLoadAddon('addons/SaveManager.lua')

if not ThemeManager or not SaveManager then
    warn("One or more addons failed to load")
end
...
local FooterText = 'mawu | ' .. gameName .. ' | ' .. Username
local Window = Library:CreateWindow({
    Title = 'mawu',
    Footer = FooterText,
    Icon = 'rbxassetid://5554236805',
    NotifySide = 'Right',
...
Library:Notify('updated/reworked by mawu', 15)
...
local ka_lastHealth = {}
local sounds = {
    Hrntai = 'https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/CongoOhioDog-SoundS/Hrntai.wav',
    Henta01 = 'https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/CongoOhioDog-SoundS/henta01.wav',
    Kitty = 'https://raw.githubusercontent.com/brn3d/repostuff/main/repo_downloads/CongoOhioDog-SoundS/Kitty.mp3',
}
local hitsounds = {
...
coroutine.wrap(function()
    ...
    local function applyChams(character)
        if Settings.Visuals.Character_Chams.Enabled and character then
            ...
            local chamColor = Settings.Visuals.Character_Chams.Color or Color3.fromRGB(255, 255, 255)
            local chamMaterial = Settings.Visuals.Character_Chams.Material or Enum.Material.ForceField
            
            -- List of body parts to apply chams to
            ...
            -- Apply to main body parts first
            ...
            -- Apply to accessories (hair, etc)
            for _, accessory in pairs(character:FindFirstChildOfClass('Accessory') or {}) do
                if accessory then
            ...
            -- Hide face decal for cleaner neon effect
            local head = character:FindFirstChild('Head')
            if head then
        ...
        if Settings.Visuals.Weapon_Chams.Enabled and character then
            local weaponColor = Settings.Visuals.Weapon_Chams.Color or Color3.fromRGB(255, 255, 255)
            local weaponMaterial = Settings.Visuals.Weapon_Chams.Material or Enum.Material.Neon
            local Gun = character:FindFirstChildOfClass('Tool')

            if Gun then
                for i, v in pairs(Gun:GetChildren())do
                    if v:IsA('MeshPart') then
                        v.Material = weaponMaterial
                        v.Color = weaponColor
                        v.TextureID = ''
                    end
    ...
    local function onChamsCharacter(character)
        ...
        -- Reapply chams when new tool is added
        chamsConnections.toolAdded = character.ChildAdded:Connect(function(child)
        ...
        -- Reapply chams when tool is removed
        chamsConnections.toolRemoved = character.ChildRemoved:Connect(function(child)
            if child:IsA('Tool') then
                task.wait(0.1)
                applyChams(character)
            end
...
NewConnection(RunService.Heartbeat, function(dt)
    ...
    NewConnection(RunService.Heartbeat, function(dt)
        ...
        if useDesync and hrp then
            ...
            elseif matchacc.StandaloneAutoStomp.Enabled and not (AutoKillActive or AutoArmorActive or headshots.AutoLoadout.CurrentBuying or BuyingActive or AutoHealActive) then
                -- Standalone autostomp: stomp ANY knocked down player without range limit
                local shortestDistance = math.huge
                local autoStompTarget
                for _, player in pairs(players:GetPlayers()) do
                    if player ~= localPlayer and player.Character and player.Character:FindFirstChild('Head') and KnockCheck(player) and not player.Character:FindFirstChildWhichIsA('ForceField') then
                        local distance = (player.Character.HumanoidRootPart.Position - hrp.Position).Magnitude
                        if distance < shortestDistance then
                            shortestDistance = distance
                            autoStompTarget = player
                        end
                    end
                end
                if autoStompTarget then
                    local bodyEffects = autoStompTarget.Character:FindFirstChild('BodyEffects')
                    local isDead = bodyEffects and bodyEffects:FindFirstChild('SDeath') and bodyEffects.SDeath.Value
                    if isDead == false and localPlayer.Character.BodyEffects.Reload.Value == false then
                        hrp.CFrame = CFrame.new(autoStompTarget.Character.UpperTorso.Position + Vector3.new(0, 3, 0))
                        RunService:BindToRenderStep('RestoreStandaloneAutoStomp', 199, function()
                            hrp.CFrame = SavedPosition
                            RunService:UnbindFromRenderStep('RestoreStandaloneAutoStomp')
                        end)
                        if matchacc.HitEffects.HitNotifications then
                            if tick() - t >= 3 then
                                t = tick()
                                Library:Notify('matcha - Auto Stomped: ' .. autoStompTarget.Name, matchacc.HitEffects.HitNotificationsTime)
                            end
                        end
                        MainEvent:FireServer('Stomp')
                    end
...
coroutine.wrap(function()
    ...
    MiscBox:AddToggle('StandaloneAutoStomp', {
        Text = 'Auto Stomp (Any Target)',
        Default = false,
        Callback = function(v)
            matchacc.StandaloneAutoStomp.Enabled = v
        end,
    })
...
coroutine.wrap(function()
    ...
    local LightingGroup = Tabs.World:AddLeftGroupbox('Lighting')
        ...
        if DefaultSky then
            ...
            -- Fallback defaults if no Sky object exists
            LightingSettings.DefaultSkyboxBk = ''
            LightingSettings.DefaultSkyboxDn = ''
            LightingSettings.DefaultSkyboxFt = ''
            LightingSettings.DefaultSkyboxLf = ''
            LightingSettings.DefaultSkyboxRt = ''
            LightingSettings.DefaultSkyboxUp = ''
        end
